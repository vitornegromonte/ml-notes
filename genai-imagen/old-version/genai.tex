...
\begin{abstract}
   This notebook will serve as a comprehensive repository for all my notes on an introductory course about Deep Generative Models, with a particular focus on image generation. The content will cover the following key topics: Style Transfer, Autoencoders, Variational Autoencoders, Generative Adversarial Networks and Diffusion Models. In the future I'll provide notes about Energy-based models and Boltzman Machines.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{resources/ae.png}
    \caption{Exemplo genérico de arquitetura de Autoencoders}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{resources/vae.png}
    \caption{Arquitetura de um Variational Autoencoder}
    \label{fig:enter-label}
\end{figure}
...
\subsubsection{Como Funciona o Stable Diffusion}
...
Onde:
\begin{itemize}
    \item $\theta_g$ são os parâmetros do gerador.
    \item $\theta_d$ são os parâmetros do discriminador.
    \item $D(x)$ é a probabilidade estimada pelo discriminador de que $x$ seja real.
    \item $G(z)$ é a amostra gerada pelo gerador a partir do vetor de ruído $z$.
\end{itemize}
...
\end{itemize}
\subsubsection{Conexão com a Divergência de Jensen-Shannon (Demonstrado no Artigo Original)}
