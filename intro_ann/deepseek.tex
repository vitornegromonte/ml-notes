...
\end{tikzpicture}
\end{adjustbox}
\caption{Spatial evolution of perceptron weight updates during shape classification. 
...
corners and center become strongly inhibitory (muted earth tones).}
\end{figure}
...
\begin{tikzpicture}[scale=0.6]
    \begin{axis}[width=4.5cm, height=3cm, xlabel=Model capacity, ylabel=Error, domain=0:1, samples=50, axis lines=middle, enlargelimits]
        \addplot[wp-primary, thick] {0.1 + 0.5*(x-0.2)^2}; % bias^2
        \addplot[wp-accent, thick] {0.5*exp(5*(x-0.6))}; % variance
        \addplot[wp-muted, thick, dashed] {0.1 + 0.5*(x-0.2)^2 + 0.5*exp(5*(x-0.6))}; % total
