...
\begin{tikzpicture}[scale=.6, thick]
    ...
    \begin{scope}[shift={(9, 0)}] % Shift to the right
        ...
        \foreach \i in {0,...,6} {
            ...
            \pgfmathsetmacro{\xstart}{\i * 6.28 / 7}
            \pgfmathsetmacro{\xend}{(\i + 1) * 6.28 / 7}
        ...
        % Add labels
        \node[above] at (6.28, 0) {\textcolor{wp-text}{$\sin(x)$}};
        \node[above] at (3.14, 1.3) {\textcolor{wp-text}{7 linear regions}};

        % Mark key points
        \foreach \i in {0,...,10} {
            \pgfmathsetmacro{\x}{\i * 6.28 / 7}
            \pgfmathsetmacro{\y}{sin(\x r)}
            \fill[wp-primary] (\x, \y) circle (0.5pt);
        }
    \end{scope}
\end{tikzpicture}
\caption{Visualization of the Universal Approximation Theorem with 5 and 7 linear regions}
\label{fig:UAT}
\end{figure}
