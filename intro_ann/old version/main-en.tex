...
Nearly every modern artificial neural network, from simple classifiers to sprawling deep learning architectures, traces its origins to a simple computational unit called the \textbf{perceptron} \footnote[1]{While most neural networks rely on perceptron-like units, alternative paradigms exist, such as Hebbian learning (inspired by synaptic plasticity) and the recently revived Kolmogorov-Arnold Network (KAN), which bypasses traditional neuron structures entirely.}.
...
\subsubsection{Mathematical Foundation of Perceptrons}

Given inputs $x_1, x_2, ..., x_n$ the perceptron's output ($X$) is governed by two steps:
\begin{enumerate}
...
Proposed by McCulloch and Pitts in 1943, the Perceptron can be used to represent many boolean functions, indeed, it can represent all of the primitive boolean functions (AND, OR, NAND and NOR). Unfortunately, however, some boolean functions can be represented by a single perceptron, such as the XOR function whose value is 1 $\leftrightarrow x_1 \neq x_2$. This limitation of a single perceptron unit to not be able to represent a XOR function started the first Computational Intelligence Winter.


\subsubsection{Geometric Interpretation of the Perceptron}
...
\centering
    ...
    \draw[white, thick] (0.5,3.5) -- (3.5,0.5) node[midway, left]{$\mathbf{w}\cdot\mathbf{x}=0$};

    % Weight vector (normal to boundary)
    \draw[->, Orchid, very thick] (2,2) -- (3,3) node[above right]{$\mathbf{w}$};

    % Positive examples (output +1)
    \filldraw[Orchid] (1,3) circle (2pt) node[above right]{};
    \filldraw[Orchid] (2,2.5) circle (2pt) node[above right]{};
    \filldraw[Orchid] (3,2) circle (2pt) node[above right]{};

    % Negative examples (output -1)
    \filldraw[orange] (1,1) circle (2pt) node[below left]{};
    \filldraw[orange] (2,0.5) circle (2pt) node[below left]{};
    \filldraw[orange] (3,0) circle (2pt) node[below left]{};

    % Class labels
    \node[Orchid] at (3.7,3) {Class $+1$};
    \node[orange] at (3.7,1) {Class $-1$};
...
The "XOR Problem" is a classical problem in Computational Intelligence, proposed initially by Marvin Minsky in his book "Perceptron" (1969). It's the problem of using a Perceptron to predict the output of XOR logic gates given two binary outputs. A XOR function should only return true if the two inputs are not equal and a false if they are equal.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \begin{axis}[
                ...
                minor tick num=-3,
                enlargelimits=true]
              \node[point,label={[label distance=0.3cm,text=Orchid]135:$0$},Orchid] at (axis cs:1,0) {};
              \node[point,label={[label distance=0.3cm,text=Orchid]-45:$0$},Orchid] at (axis cs:0,1) {};
              \node[point,label={[label distance=0.3cm,text=MidnightBlue]45:$1$},MidnightBlue] at (axis cs:0,0) {};
              \node[point,label={[label distance=0.3cm,text=MidnightBlue]-135:$1$},MidnightBlue] at (axis cs:1,1) {};
            \end{axis}
        \end{tikzpicture}
    ...
    \end{subfigure}
    % Combined caption
    \caption{XOR operation represented as a truth table (a) and a visualization (b). The plot shows the XOR output for different inputs, where $0$s are marked in \textcolor{Orchid}{Orchid} and $1$s are marked in \textcolor{MidnightBlue}{MidnightBlue}.}
    \label{fig:xor_combined}
\end{figure}

The structure of a perceptron unit is limited to separating the data point with a single line and since the XOR inputs are not linearly separable. This can be solved by adding an extra layer of neurons to the network, called hidden-layer, this architecture is called Multilayer perceptron (next section). 
    

\subsection{Gradient Descent and Stochastic Gradient Descent}
\subsubsection{Perceptron Training Rule}
One way to learn an acceptable weight vector is to begin with random weights, the  iteratively apply the perceptron to each training example. modifying the perceptron weight whenever it miss classifies an example. This process is repeated, iterating throug the training examples as many times as needed until the perceptron classifies all training examples correctly. Weights are modified at each step according to the perceptron training rule, which revies the weight $w_i$ associated with input $x_i$ according to the rule
...
The key idea behind the delta rule is to use gradient descent to search the hypothesis space of possible weight vectors to find the weights that best fit the training examples. This rules sets the basis for the Backpropagation Algorithm.
...
\end{equation}
where $D$ is the set of training examples, $t_d$ is the target output for training example $d$, and $o_d$ is the output of the linear unit for training example. 

\subsubsection{Gradient Descent}

To better understand the Gradient Descent, it's helpful to visualize the entire hypothesis space of possible weight vectors and their associate E values (Figure \ref{fig:GD}). The Gradient Descent search determines a weight vector that minimizes $E$ by starting with and arbitrary initial weight vector, then repeatedly modifying it in small steps. At each step, the weight vector is altered in the direction that produces the steepest descent along the error surface depicted in Figure \ref{fig:GD}. 
...
Based on that, we can calculate the direction of steepest descent along the error surface ($E$) computing the derivative of E with respect to each component of the vector $\Vec{w}$. This vector derivative is called the gradient of $E$ with respect to $\Vec{w}$, written $\nabla E(\Vec{w})$.
\begin{equation}
    \nabla E(\Vec{w}) \equiv 
    \left[ \frac{\partial E}{\partial w_o}, \frac{\partial E}{\partial w_1}, ..., \frac{\partial E}{\partial w_n} \right]
\end{equation}
And $\nabla E(\Vec{w})$ is itself a vector, whose components are the partial derivatives of $E$ with respect to each of the $w_i$. When interpreted as a vector in weight space, the gradient specifies the direction that produces the steepest increase in $E$.
...
To efficiently calculate the gradient at each step, we need to obtain the derivatives of $\frac{\partial E}{\partial w_i}$ by differentiating E from Equation \ref{eq:7} as:
\begin{equation}
    \begin{split}
...
where $x_id$ denotes the single output component $x_i$ for training example $d$. With all that, we now have an equation that gives $\frac{\partial E}{\partial w_i}$ in terms of the linear unit inputs $x_{id}$, outputs $O_d$, and target values $t_d$ associated with the training examples. With this, we have:

\begin{equation}
    \Delta w_i = \eta \sum_{d \in D} (t_d - o_d)x_{id}
\end{equation}

\subsubsection{Stochastic Approximation to Gradient Descent}
Due to it's computation complexity, usually, we do not use directly GD. The need of using a whole set at each iteration during training, makes it more computationally expensive to use GD. Based on that, we do use an stochastic approximation to gradient descent, mostly called stochastic gradient descent or SGD.
In SGD, instead of using the entire dataset for each iteration, only a single random sub-set is selected to calculate the gradient and update model parameters. The random selection introduces randomness into the optimization process (stochastic stands for being well-described by a random probability distribution). Mathematically it can be described as

\begin{equation}
    w_i := w_i - \eta \nabla Q_i (w)
\end{equation}

The randomness of the technique can also provoke some noise in the gradient, which some big variations in the gradient.
...
For learning purposes, we'll address the previous network (perceptron) as a Shallow Neural Network, we call it that way, because the only way to increase the number of neurons it's adding them \textit{"vertically"} due to the nature of a single layer of neurons. More over, in this chapter we'll dive into the evolution of these concepts, from Shallow Neural Networks to Deep Neural Networks and it's impacts in the field.

The simple way of making a neural network deeper it's to compositing shallow neural networks. Basically, the output of the first unit becomes the input of the second unit. 

In first sight, we need to decide what type of unit shell we use as the basis for constructing multilayer networks. Initially, we might be tempted to choose the linear units discussed before, for which we already derived a gradient descent learning rule. However, multiple layers of cascaded linear units still produce only linear functions. What we need is a unit whose output is a nonlinear function of its inputs, but whose output is also a differentiable function of its inputs , like a sigmoid unit or a tanh unit or a ReLU unit, any function can be use since it covers all the requirements described above.
...
\begin{itemize}
    \item Each training example is a pair of the form $\langle \Vec{x}, \Vec{t} \rangle$, where $\Vec{x}$ is the vector of network input values, and $\Vec{t}$ is the vector of target network output values.
...
\begin{tikzpicture}[scale=0.6, thick]
    ...
    \begin{scope}[shift={(0, 0)}] % Shift to the left for alignment
        ...
        % Draw the sine wave
        \draw[white, thick, domain=0:6.28, samples=100, smooth] 
            plot (\x, {sin(\x r)});

        % Draw linear approximation
        \draw[Orchid, thick] (0, 0) -- (1.57, 1);
        \draw[Orchid, thick] (1.57, 1) -- (3.14, 0);
        \draw[Orchid, thick] (3.14, 0) -- (4.71, -1);
        \draw[Orchid, thick] (4.71, -1) -- (6.28, 0);

        % Add labels
        \node[above] at (6.28, 0) {\textcolor{white}{$\sin(x)$}};
        \node[above] at (3.14, 1) {\textcolor{white}{5 linear regions}};

        % Mark key points
        \foreach \x/\y in {0/0, 1.57/1, 3.14/0, 4.71/-1, 6.28/0} {
            \fill[Orchid] (\x, \y) circle (1pt);
        }
    \end{scope}
    ...
    \begin{scope}[shift={(9, 0)}] % Shift to the right
        ...
        \foreach \i in {0,...,19} {
            ...
            \draw[Orchid, thick] (\xstart, \ystart) -- (\xend, \yend);
        }

        % Add labels
        \node[above] at (6.28, 0) {\textcolor{white}{$\sin(x)$}};
        \node[above] at (3.14, 1.3) {\textcolor{white}{20 linear regions}};

        % Mark key points
        \foreach \i in {0,...,20} {
            \pgfmathsetmacro{\x}{\i * 6.28 / 20}
            \pgfmathsetmacro{\y}{sin(\x r)}
            \fill[Orchid] (\x, \y) circle (0.5pt);
        }
    \end{scope}
\end{tikzpicture}
...
To easily understand \textit{UAT}, we can visualize how it works in Figure \ref{fig:UAT}
